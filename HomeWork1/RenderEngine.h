...
private:
	...
	void RT_LoadMesh(const Ogre::String& _meshName);
	void RT_CreateSphere(const size_t& index,float radius);
	void RT_SetupDefaultLight();
	void RT_OscillateCamera(float time);
	void RT_MovePlanet(const UINT32& index, const Ogre::Vector3& pos);
