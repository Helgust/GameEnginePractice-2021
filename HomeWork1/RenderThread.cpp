...
void RenderThread::ProcessCommands()
{
	...
	while (n < m_Commands[m_nCurrentFrame].Ñapacity())
	{
		...
		case eRC_CreateSphere:
		{
			...
			float radius = ReadCommand<float>(n);
			m_pRenderEngine->RT_CreateSphere(index,radius);
			break;
		}
		...
		case eRC_MovePlanet:
		{
			...
			m_pRenderEngine->RT_MovePlanet(index, {x,y,z});
			break;
		}
		}
	}

	m_Commands[m_nCurrentFrame].Ñlear();
}
...
byte* RenderThread::AddCommand(RenderCommand eRC, size_t nParamBytes)
{
	...
	byte* ptr = m_Commands[m_nFrameFill].Resize(m_Commands[m_nFrameFill].Ñapacity() * sizeof(byte) + cmdSize);
...
void RenderThread::RC_LoadMesh(const Ogre::String& meshName)
{
	...
	LOADINGCOMMAND_CRITICAL_SECTION;
	byte* p = AddCommand(eRC_LoadMesh, 0);
}

void RenderThread::RC_CreateSphere(const size_t& index,float radius)
{
	if (IsRenderThread())
	{
		m_pRenderEngine->RT_CreateSphere(index,radius);
	...
	LOADINGCOMMAND_CRITICAL_SECTION;
	byte* p = AddCommand(eRC_CreateSphere, sizeof(DWORD) + sizeof(float));
	AddDWORD(p, index);
	AddFloat(p, radius);
}
...
void RenderThread::RC_MovePlanet(const UINT32& index, const Ogre::Vector3& pos)
{
	if (IsRenderThread())
	{
		m_pRenderEngine->RT_MovePlanet(index, pos);
		return;
	}
